...
import { sidebarPlatform, sidebarWorkspaces, user } from "../_data/mock";
    ...
    <aside className="flex h-screen w-60 shrink-0 flex-col bg-sidebar p-2 text-sidebar-foreground">
      ...
      <nav className="flex flex-col gap-0.5" aria-label="Platform navigation">
        {sidebarPlatform.map((item) => {
          ...
          return (
            <SidebarItem key={item.id} state={item.active ? "active" : "default"}>
      ...
      {/* User card */}
      <button className="flex h-13 items-center gap-2 rounded-md p-2 text-left transition-colors hover:bg-sidebar-accent">
